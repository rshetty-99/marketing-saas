...
export default async function AnalyticsPage() {
  ...
  const canExport = await hasPermission('workspace', result.member.role, 'analytics.export_reports');

  const metrics = await getDashboardMetrics(result.workspaceId, 'last_30_days');
      ...
      <header className="sticky top-0 z-10 flex h-14 shrink-0 items-center gap-2 border-b border-border bg-background/95 backdrop-blur-sm px-4">
        ...
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbPage className="font-ui text-sm">Analytics</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      </header>
