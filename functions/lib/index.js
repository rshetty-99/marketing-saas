...
exports.contentDecayMonitor = exports.rankingTracker = exports.calendarNotifications = exports.approvalEscalation = exports.publishScheduled = exports.metricsPoller = exports.tokenRefresh = exports.trialEnforcement = void 0;
...
// ═══════════════════════════════════════════════════════════
exports.trialEnforcement = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running trial enforcement check');
    ...
    for (const ws of expiredTrials.docs) {
        ...
        locked++;
        firebase_functions_1.logger.info(`Soft-locked workspace: ${ws.id} (${ws.data().name})`);
    }
...
// ═══════════════════════════════════════════════════════════
exports.tokenRefresh = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running token refresh check');
...
// ═══════════════════════════════════════════════════════════
exports.metricsPoller = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running metrics poller');
    ...
    for (const ws of workspaces.docs) {
        ...
        if ((await snapshotRef.get()).exists)
            continue;
...
// ═══════════════════════════════════════════════════════════
exports.publishScheduled = (0, firestore_1.onDocumentCreated)('workspaces/{workspaceId}/publish_jobs/{jobId}', async (event) => {
    const data = event.data?.data();
    if (!data || data.status !== 'scheduled')
        return;
    const scheduledAt = data.scheduledAt?.toDate?.();
    if (!scheduledAt || scheduledAt > new Date()) {
        firebase_functions_1.logger.info(`Publish job ${event.params.jobId} scheduled for future`);
        return;
    }
    ...
    firebase_functions_1.logger.info(`Published job ${event.params.jobId} to ${platforms.length} platforms (mock)`);
});
// ═══════════════════════════════════════════════════════════
// 5. APPROVAL ESCALATION — manually callable until go-live
// Go-live schedule: hourly at :30 ('30 * * * *')
// ═══════════════════════════════════════════════════════════
exports.approvalEscalation = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running approval escalation check');
    const now = new Date();
    let escalated = 0;
...
// ═══════════════════════════════════════════════════════════
exports.calendarNotifications = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running calendar notification check');
    const now = new Date();
...
// ═══════════════════════════════════════════════════════════
exports.rankingTracker = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running ranking tracker');
    let tracked = 0;
    const workspaces = await db.collection('workspaces').where('status', 'in', ['trial', 'active']).get();
    for (const ws of workspaces.docs) {
        ...
        for (const keyword of keywords) {
            await db.collection('workspaces').doc(ws.id).collection('seo_reports').add({
                ...
                rankings: [{ keyword, url: `https://example.com/${keyword.replace(/\s+/g, '-')}`, position: Math.floor(Math.random() * 50) + 1, searchVolume: Math.floor(Math.random() * 10000) + 100, checkedAt: admin.firestore.FieldValue.serverTimestamp() }],
...
// ═══════════════════════════════════════════════════════════
exports.contentDecayMonitor = (0, https_1.onRequest)(async (_req, res) => {
    firebase_functions_1.logger.info('Running content decay monitor');
    let decayDetected = 0;
    const workspaces = await db.collection('workspaces').where('status', 'in', ['trial', 'active']).get();
    for (const ws of workspaces.docs) {
        ...
        if (snapshots.size < 2)
            continue;
        const latest = (snapshots.docs[0].data().rankings ?? []);
        const previous = (snapshots.docs[1].data().rankings ?? []);
        for (const lr of latest) {
            const pr = previous.find((p) => p.keyword === lr.keyword);
            if (pr && lr.position - pr.position > 10)
                decayDetected++;
        }
    }
